...
stop_words.update(["amp", "", "th", "via", "r"])
...
tweets['text'] = tweets['text'].apply(lambda x: ' '.join([item for item in x.split(" ") if item not in stop_words]))

tweets['text'] = tweets.text.apply(lemmatize_text)

tweets['text'] = tweets['text'].apply(lambda x: ' '.join([item for item in x]))
